...
class ServerObject
{
    ...
    function __construct()
    {
        ...
        if (defined('DB_CA_PATH')) {
            ...
            // Try to match the connection charset to the database charset to avoid
            // "Conversion from collation ... impossible for parameter" errors.
            $cs_res = $this->sql->query("SELECT @@character_set_database AS cs");
            if ($cs_res) {
                $cs_row = $cs_res->fetch_assoc();
                $db_cs = $cs_row['cs'] ?? null;
                if ($db_cs) {
                    $this->sql->set_charset($db_cs);
                } else {
