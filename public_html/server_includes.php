...
#				$res = $this->sql->query("SELECT distinct($table.id) FROM $table_sources JOIN $table ON $table_sources.id = $table.id WHERE ($table_sources.source like '%$searchValue%' OR JSON_SEARCH( lower(yara->'$.yara'), 'all', lower('$searchValue')) IS NOT NULL) ORDER BY $table.id DESC");

#				$res = $this->sql->query("SELECT distinct($table.id) FROM $table_sources JOIN $table ON $table_sources.id = $table.id WHERE ($table_sources.source like '%$searchValue%' OR JSON_SEARCH( lower(yara->'$.yara'), 'all', lower('$searchValue')) IS NOT NULL) ORDER BY $table.id DESC limit 1, 5002");
